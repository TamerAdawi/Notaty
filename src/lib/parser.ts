...
export interface ParsedNote {
  ...
  tags: string[];
  meta: NoteMeta;
  done?: boolean; // set when the text ended with a ✅-style "done" mark
}
...
const CATEGORY_RULES: CategoryRule[] = [
  { name: 'Work', icon: '💼', words: ['meeting', 'client', 'project', 'deadline', 'boss', 'report', 'presentation', 'office', 'team', 'deploy', 'invoice', 'standup', 'sprint', 'jira', 'ticket', 'email', 'colleague', 'manager', 'demo', 'launch', 'shift', 'work', 'اجتماع', 'مشروع', 'دوام', 'الشغل', 'شغل', 'شيفت', 'شفت'] },
  { name: 'Health', icon: '🩺', words: ['doctor', 'dentist', 'gym', 'workout', 'medicine', 'pills', 'pill', 'meds', 'run', 'running', 'exercise', 'water', 'sleep', 'therapy', 'clinic', 'hospital', 'vitamins', 'training', 'physio', 'صحة', 'صحية', 'طبيب', 'دكتور', 'نادي', 'تمرين', 'دواء', 'نوم', 'سبلمنتس', 'حلاق'] },
  ...
  { name: 'Finance', icon: '💰', words: ['pay', 'bill', 'bills', 'rent', 'money', 'bank', 'budget', 'tax', 'taxes', 'salary', 'transfer', 'loan', 'subscription', 'renew', 'expenses', 'فاتورة', 'ايجار', 'فلوس', 'بنك', 'راتب', 'ادفع', 'مصاريف', 'حساب'] },
  { name: 'Study', icon: '📚', words: ['study', 'exam', 'homework', 'read', 'course', 'learn', 'lecture', 'assignment', 'quiz', 'test', 'revise', 'revision', 'chapter', 'university', 'school', 'دراسة', 'امتحان', 'مذاكرة', 'محاضرة', 'واجب', 'اقرأ', 'تعلم', 'الجامعة', 'جامعة', 'التخنيون', 'تخنيون', 'العبري', 'عبري'] },
...
const ACTION_VERBS = [
  // English
  'buy', 'call', 'email', 'send', 'finish', 'do', 'fix', 'book', 'pay', 'schedule', 'submit',
  'clean', 'wash', 'cook', 'order', 'renew', 'get', 'make', 'prepare', 'check', 'review', 'reply',
  'meet', 'pick', 'go', 'text', 'message',
  // Arabic (Levantine imperatives)
  'اعمل', 'اعملي', 'سوي', 'سوّي', 'ادخل', 'قول', 'قولي', 'احكي', 'احكيلو', 'حط', 'حطي', 'حطّ',
  'رتب', 'رتّب', 'رتبي', 'لاقي', 'لاقيلي', 'اشتري', 'جدد', 'جدّد', 'انقل', 'نزل', 'نزّل', 'زبط',
  'زبّط', 'اعزل', 'عبي', 'عبّي', 'افحص', 'اعرف', 'خذ', 'توخذ', 'اسأل', 'اسالي', 'احضر', 'شوف',
  'روح', 'جيب', 'صلح', 'صلّح', 'كمل', 'كمّل', 'ابدأ', 'ابدا', 'سجل', 'سجّل', 'ابعت', 'بعت', 'رد',
  'ردّ', 'اقرأ', 'اقرا', 'شغل', 'شغّل', 'تعلم', 'تعلّم', 'احجز', 'ادفع', 'اتصل', 'راجع', 'ارسل',
  'نظف', 'نظّف', 'اغسل', 'اطبخ', 'جهز', 'جهّز', 'حضّر', 'حضر', 'حدد', 'حدّد', 'اطلب', 'اعد', 'عيد', 'كرر',
];
const TASK_PHRASES = ['need to', 'have to', 'must', 'todo', 'to-do', 'to do', 'لازم', 'يجب', 'بدي اعمل', 'بدي'];
const IDEA_WORDS = ['idea:', 'idea ', 'what if', 'maybe i', 'maybe we', 'concept', 'thought:', 'imagine', 'فكرة'];
const REMINDER_WORDS = ['remind me', 'reminder', "don't forget", 'dont forget', 'remember to', 'ذكرني', 'ذكّرني', 'تذكير', 'تذكر', 'لا تنسى', 'متنساش'];
...
// Arabic weekday names → JS day index (0 = Sunday). Match the root so attached
// prefixes (ال / لل / و / ب…) still hit, e.g. "للجمعة", "والاربعا".
const AR_WEEKDAYS: { i: number; re: RegExp }[] = [
  { i: 0, re: /الأحد|الاحد|للأحد|للاحد/ }, // article-required (avoids واحد/احداث)
  { i: 1, re: /اثنين|اتنين/ },
  { i: 2, re: /ثلاثاء|ثلاثا/ },
  { i: 3, re: /أربعا|اربعا/ },
  { i: 4, re: /خميس/ },
...
function parseDueDate(rawText: string): Date | null {
  ...
  if (/\btoday\b|\btonight\b|اليوم/.test(text)) {
    ...
    if (!base) {
      // Arabic weekday names (e.g. يوم الاربعا، الجمعة الجاي) → next occurrence
      for (const w of AR_WEEKDAYS) {
        if (w.re.test(text)) {
          base = startOfDay(now);
          const diff = (w.i - base.getDay() + 7) % 7 || 7;
          base.setDate(base.getDate() + diff);
          break;
        }
      }
...
function detectType(text: string): { type: NoteType; meta: NoteMeta } {
  ...
  // 3. Question (but not when it's really a task — "شو لازم اعرف…" has لازم)
  const firstWord = trimmed.toLowerCase().split(/\s+/)[0]?.replace(/[^\p{L}]/gu, '') || '';
  if (
    trimmed.endsWith('?') ||
    (QUESTION_STARTS.includes(firstWord) &&
      !startsWithActionVerb(text) &&
      !includesAny(lower, TASK_PHRASES))
  ) {
...
export function parseNote(rawText: string): ParsedNote {
  // A trailing ✅/✔/✓/☑ means the user marked it done — record it and strip it.
  let content = rawText.trim();
  const done = DONE_MARK.test(content);
  if (done) content = content.replace(DONE_MARK, '').trim();
